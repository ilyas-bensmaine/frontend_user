...
const EditOfferModal = ({open, toggleModal, selectedPost}) => {
  // ** Hook 
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (selectedPost) {
          const response = await axiosInstance.get(`/api/posts/${selectedPost.id}/show_user_response`)
          try {
              reset({
                response_id : response.data.id,
                price: response.data.price,
                content: response.data.content,
                wilaya: {value:response.data.wilaya.id, label:response.data.wilaya.name, ...response.data.wilaya},
                urls: response.data.media.length ? response.data.media.map((item) => {return item.original_url}) : []
              })
          } catch (error) {
            console.log(error)
          }
  ...
  const onSubmit = async (data) => {
    console.log(data)
    try {
      await axiosInstance.post(`api/post_responses/${data.response_id}`, {...data, _method: 'patch'})
  ...
  return (
    <Fragment>
        ...
        >
            <ModalHeader className='bg-transparent' toggle={onToggle}></ModalHeader>
            <ModalBody>
            ...
            <h4 className='text-center mb-1'>Post a new offer for the post {selectedPost.title}</h4>
                ...
                <Row>
                    ...
                    <Controller
                        ...
                        render={({ formState }) => (
                        <FileUploader uploadedFiles={[...getValues('urls')]} fileExtension={'image/*'} fileMultiple={true} onAddUrl={handleAddedUrl} onDeleteUrl={handleDeletedUrl} ></FileUploader>
                        )}
                    />
                    ...
                    <Button type='reset' className='mt-2' color='secondary' outline onClick={onToggle}>
                        Discard
                    </Button>
                    </Col>
                </Row>
